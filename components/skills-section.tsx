...
const SkillCategory = ({ category, index }: { category: (typeof skillCategories)[0]; index: number }) => {
    ...
    const tl = gsap.timeline({
      scrollTrigger: {
        ...
        onComplete: () => {
          ...
          ScrollTrigger.getById(`skill-card-${index}`)?.kill()
        },
      },
    })
  ...
  return (
    ...
    >
      <div ref={headerRef} className="flex items-center gap-1.5 md:gap-2 mb-2 md:mb-4 min-h-[24px] md:min-h-[28px]">
        <span ref={iconRef} className="text-xs md:text-base flex-shrink-0 will-change-transform">
          {category.icon}
        </span>
        <h3
          ref={titleRef}
      ...
      <div ref={skillsListRef} className="space-y-2 md:space-y-3">
        {category.skills.map((skill, skillIndex) => (
          <div key={skill} className="flex items-center gap-2 md:gap-3 will-change-transform">
            <div className="w-1 h-1 md:w-1.5 md:h-1.5 bg-[#00D2BE] rounded-full flex-shrink-0 opacity-80" />
            <span className="text-neutral-300 font-f1 text-xs leading-relaxed">{skill}</span>
          </div>
        ))}
      </div>
    </div>
...
export default function SkillsSection() {
        ...
        { textContent: "0" },
        {
          ...
          onUpdate: function () {
            if (finalValue.includes("+")) {
              ...
              element.textContent = Number.parseFloat(this.targets()[0].textContent).toFixed(2)
            }
          },
        },
      )
  ...
  return (
    <section ref={sectionRef} id="skills" className="section container mx-auto py-12 md:py-24 px-2 md:px-4">
      <div ref={headerRef} className="text-center mb-8 md:mb-12">
        ...
        <p className="text-neutral-400 font-f1 max-w-xl mx-auto text-xs md:text-sm px-2">
          Core technologies and capabilities powering high-performance digital solutions.
        </p>
      </div>
      ...
      <div className="max-w-4xl mx-auto mb-8 md:mb-12">
        <div className="bg-[#0f0f0f] border border-neutral-800 rounded-lg p-3 md:p-6">
          ...
          <div ref={softSkillsRef} className="flex flex-wrap justify-center gap-2 md:gap-3">
      ...
      <div ref={metricsRef} className="flex justify-center gap-4 md:gap-8 text-center">
        <div>
          <div data-metric="20+" className="text-lg md:text-xl font-f1-bold text-[#00D2BE]">
            20+
          </div>
          <div className="text-xs text-neutral-400 font-f1">TECHNOLOGIES</div>
        </div>
        <div>
          <div data-metric="3.75" className="text-lg md:text-xl font-f1-bold text-[#00D2BE]">
            3.75
          </div>
          <div className="text-xs text-neutral-400 font-f1">GPA</div>
        </div>
        <div>
          <div data-metric="280+" className="text-lg md:text-xl font-f1-bold text-[#00D2BE]">
            280+
          </div>
          <div className="text-xs text-neutral-400 font-f1">STUDENTS TAUGHT</div>
        </div>
      </div>
    </section>
  )
