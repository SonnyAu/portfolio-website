...
export default function AdvancedLoadingSystem({ onComplete }: LoadingSystemProps) {
  ...
  useEffect(() => {
    ...
    const initializeLoadingSystem = async () => {
      ...
      const loadingSteps = [
        { progress: 10, text: "ENGINE STARTUP SEQUENCE", rpm: 1000, speed: 0, temp: 25 },
        { progress: 25, text: "LOADING TELEMETRY SYSTEMS", rpm: 3000, speed: 0, temp: 40 },
        { progress: 40, text: "CALIBRATING SENSORS", rpm: 5000, speed: 50, temp: 60 },
        { progress: 60, text: "OPTIMIZING PERFORMANCE", rpm: 8000, speed: 120, temp: 75 },
        { progress: 80, text: "WARMING UP SYSTEMS", rpm: 12000, speed: 200, temp: 85 },
        { progress: 95, text: "FINAL SYSTEM CHECK", rpm: 15000, speed: 280, temp: 95 },
        { progress: 100, text: "READY FOR RACE START", rpm: 8000, speed: 0, temp: 90 },
      ]

      // Simulate realistic F1 telemetry during loading
      telemetryInterval = setInterval(() => {
        setTelemetryData((prev) => ({
          rpm: Math.max(0, prev.rpm + (Math.random() - 0.5) * 1000),
          speed: Math.max(0, prev.speed + (Math.random() - 0.5) * 20),
          gear: Math.max(1, Math.min(8, prev.gear + (Math.random() > 0.8 ? (Math.random() > 0.5 ? 1 : -1) : 0))),
          temp: Math.max(20, Math.min(120, prev.temp + (Math.random() - 0.5) * 5)),
        ...
        timelineInstance.to(
          {},
          {
            ...
            onUpdate: function () {
              ...
              setTelemetryData((prev) => ({
                ...prev,
                rpm: gsap.utils.interpolate(prev.rpm, step.rpm, this.progress()),
                speed: gsap.utils.interpolate(prev.speed, step.speed, this.progress()),
                temp: gsap.utils.interpolate(prev.temp, step.temp, this.progress()),
              }))
            },
          },
        )
    ...
    >
      ...
      <div className="flex flex-col items-center justify-center space-y-8 relative z-10">
        ...
        <div ref={textRef} className="text-center">
          ...
          <div className="text-sm md:text-lg text-neutral-400 font-f1 mb-4">{loadingText}</div>

          {/* Live telemetry display */}
          <div ref={telemetryRef} className="flex justify-center gap-4 text-xs font-f1">
            <div className="bg-black/60 border border-[#00D2BE]/30 rounded px-2 py-1">
              <span className="text-[#00D2BE]">RPM:</span>{" "}
              <span className="text-red-400">{Math.round(telemetryData.rpm)}</span>
            </div>
            <div className="bg-black/60 border border-[#00D2BE]/30 rounded px-2 py-1">
              <span className="text-[#00D2BE]">KPH:</span>{" "}
              <span className="text-yellow-400">{Math.round(telemetryData.speed)}</span>
            </div>
            <div className="bg-black/60 border border-[#00D2BE]/30 rounded px-2 py-1">
              <span className="text-[#00D2BE]">TEMP:</span>{" "}
              <span className="text-green-400">{Math.round(telemetryData.temp)}°C</span>
            </div>
          </div>
        </div>
